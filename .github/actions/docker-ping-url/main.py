...
def set_outputs(file_path, key, value):
...
def ping_url(url, delay, max_trials):
    ...
    while trials < max_trials:
        try:
            ...
            print(f"Website {url} is reachable. Retrying in {delay} seconds...")
            time.sleep(delay)
            trials += 1
...
def run():
    ...
    set_outputs(os.getenv('GITHUB_OUTPUT'), 'url-reachable', website_reachable)
